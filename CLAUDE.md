...
| Date | Changes |
|------|---------|
| 2026-04-25 | **Auth lockout fixed: stale `COOKIE_DOMAIN=.console.blue` was preventing the browser from accepting the session cookie.** Symptom: Dean's magic-link logins were succeeding server-side (53+ rows in `admin_sessions`, 12+ unexpired rows in express-session's `session` table for user 1) but every page load returned 401 from `/api/auth/me` and bounced him to `/login`. **Diagnosis:** four-gate evidence run via `railway` CLI + curl + Neon `psql`. Gate 1 (curl POST `/api/auth/verify-magic-link` with `-c cookies.txt`) showed the wire-level Set-Cookie: `connect.sid=...; Domain=.console.blue; Path=/; ...; HttpOnly; Secure; SameSite=Lax` — and curl printed `* skipped cookie with bad tailmatch domain: console.blue`. Browser/curl rejected per RFC 6265 because `.console.blue` does not tail-match `triadblue.systems`. Gate 4 (`railway variables`) showed Railway production had `COOKIE_DOMAIN=.console.blue` left over from before the rebrand — `server/index.ts:47` reads `process.env.COOKIE_DOMAIN || undefined` so the stale env var was poisoning every Set-Cookie response. **Fix:** `railway variable delete COOKIE_DOMAIN`, then `railway redeploy -y`. Code path was already correct (falls through to `undefined` when env unset, which means no Domain attribute, which means browser uses request host = `triadblue.systems`). **Verification:** Gate 1 rerun post-deploy — Set-Cookie has no Domain attribute, `cookies.txt` contains the connect.sid bound to `triadblue.systems`, `GET /api/auth/me` returns `HTTP/2 200` with `{"user":{"id":1,"email":"systems@triadblue.systems","displayName":"Dean","role":"super_admin"}}`. **Files changed:** none — Railway env var change only. **Cosmetic cruft flagged for cleanup (does NOT affect runtime):** Railway has a user-set var `NODE-ENV` (literal HYPHEN) with value `production`. Confirmed via `railway run node -e 'console.log(process.env.NODE_ENV, process.env["NODE-ENV"])'` that `process.env.NODE_ENV` is `undefined` and `process.env["NODE-ENV"]` is `"production"` when invoked through `railway run`. **However, the live app reads `NODE_ENV=production` correctly** because `package.json`'s `start` script is literally `NODE_ENV=production node dist/index.js` — the start command sets it inline before Node boots. That's why the post-fix Set-Cookie correctly carries the `Secure` flag (`cookie.secure: process.env.NODE_ENV === "production"` evaluates true in the running container). Action item: delete the misnamed `NODE-ENV` Railway variable (it's noise, never read). |
| 2026-04-25 (cont.) | **Late-night cleanup after the auth lockout was resolved.** (1) Deleted the misnamed Railway production env var `NODE-ENV` (literal HYPHEN) that was flagged earlier today as cosmetic cruft — the live app reads `NODE_ENV` from the inline `start` script (`NODE_ENV=production node dist/index.js`), never from this Railway var; deleting it removes ambiguity from the Railway variables panel. Verified via `railway variable delete NODE-ENV` + `railway variables \| grep -i node` returning no match. (2) Uploaded canonical swipesblue og-image (source: `/Users/Shared/global assets/logo images and texts/swipesblue-com/swipesblue-og-image.png`, **1344×768 PNG, 585 884 bytes**) to `cdn.triadblue.com/brands/swipesblue/og-image.png` AND `twitter-image.png` via `scp` to Kamatera (`147.185.239.66`). The file's actual dimensions are 1344×768, not the nominal 1200×630 OG spec — the deviation is deliberate and acknowledged by Dean: every social platform that matters (Facebook/Meta, Twitter/X, LinkedIn) accepts 1344×768 (over the 600×315 minimum, aspect 1.75:1 vs. the recommended 1.91:1; scrapers downscale cleanly). Verified both URLs return `HTTP/1.1 200 OK` with `Content-Type: image/png`, `Content-Length: 585884` (matches local source byte-for-byte), and fresh `Last-Modified` within seconds of the upload. **Rsync caveat noted for future uploads:** initial attempt with `rsync -a` preserved the source mtime (Apr 17) and failed the Last-Modified freshness check; switched to `scp` so the server stamped the current mtime. **Important: this CDN upload alone does NOT close the swipesblue social-preview gap.** Swipesblue's own repo `index.html` still hardcodes `<meta property="og:image" content="/images/less_fees_more_revenue.jpg">`, and messaging-app scrapers read static HTML — they don't execute the OGA embed script. Until the swipesblue repo's `index.html` is edited to point at `https://cdn.triadblue.com/brands/swipesblue/og-image.png` and the site is redeployed, link previews on Slack/Discord/iMessage/etc. will continue to show the retired `less_fees_more_revenue.jpg`. That edit is a separate task scoped to the swipesblue repo, explicitly out of scope tonight. The `less_fees_more_revenue copy.jpg` file in the local archive is the retired pre-repositioning og-image and was not uploaded; it remains in Dean's archive untouched. (3) Read-only audit of the OGA upload handler at `server/routes/assets.ts:111` (`POST /api/assets/upload`) confirms the architectural debt called out 2026-04-22 is **unchanged**: the handler reads the raw request body into a `Buffer`, then calls `db.insert(assets).values({ data: buffer, ... })` which writes bytes into the Postgres `assets` table's `bytea` `data` column. The returned `asset.id` is later served by other routes as `/api/assets/file/<id>`. There is **no SSH/scp to `cdn.triadblue.com` anywhere in the handler.** Operational consequence: Dean cannot today use the OGA admin UI's drag-drop to put a file like swipesblue's og-image on the CDN — uploads through the UI land in Postgres bytea. Manual `scp` from a developer machine (the path used in step 2 above) remains the only working route to the CDN until the upload-handler rewrite (PENDING in this file, ~2 hours, separate session). (4) **businessblueprint.io brand-asset push to the CDN ahead of TikTok launch.** Same pattern as item (2): bypassed the broken OGA upload handler via direct `scp`, four source files in `/Users/Shared/global assets/logo images and texts/businessblueprint-io/` mapped to five CDN destinations: `Coach Blue and BBP Logo Image 1024.png` (**1024×1024**, 182 841 bytes) → `cdn.triadblue.com/brands/businessblueprint/logo-image.png`; `businessblueprint logo image and text as url.png` (1972×240, 37 224 bytes) → `logo-lockup.png`; `BBP Open graph images.png` (**1200×630**, 59 065 bytes) → `og-image.png` AND `twitter-image.png` (byte-for-byte copy); `businessblueprint logo text as url.png` (1725×240, 20 843 bytes) → `logo-text.png`. All five URLs verified `HTTP/1.1 200 OK`, `Content-Type: image/png`, Content-Length matching local source byte-for-byte. **Two ambiguities surfaced and resolved by Dean before any upload occurred:** the initial folder listing showed only a 560×560 mascot (Dean specified ~1024×1024+) and a 1200×630 og-image whose filename ended in "copy.png" (suggesting it was a Finder duplicate of a missing master). Halted, asked Dean to confirm or correct; he added the 1024×1024 master file and renamed the og-image to drop the "copy" suffix. Re-listed folder, dimensions clean, proceeded. **Same caveat as item (2) — this CDN push alone does NOT close the bb.io social-preview loop:** if `businessblueprint.io`'s own repo `index.html` hardcodes an older og:image / twitter:image path, messaging-app scrapers (which read static HTML and don't run the OGA embed script) will continue to show stale previews until the bb.io repo's `index.html` is edited to point at `https://cdn.triadblue.com/brands/businessblueprint/og-image.png` and the site is redeployed. That edit is scoped to the businessblueprint repo, explicitly out of scope tonight per Dean. Tonight's CDN-side push gives the bb.io repo a clean target URL to point at when that repo edit happens. |
| 2026-04-23 | No new code commits. Verified yesterday's CLAUDE.md update (commit `c611b06`) is live on `origin/main` — file header reads `Last updated: 2026-04-22` and full 6-row changelog is present on the remote. Open pending items from 4/22 unchanged: (1) OGA admin upload handler still writes to Postgres `assets` table instead of SCP'ing to `cdn.triadblue.com` (architectural debt explicitly documented in this file); (2) spoke-site `index.html` files still hardcode `og:image` / `twitter:image` tags — messaging-app scrapers read static HTML and don't run the OGA JS overlay, so link previews still show stale images until templates are edited to point at CDN URLs + redeployed; (3) spoke-site header logos still bundle from repo `attached_assets/`, not CDN — needs `brand-assets.ts` module per repo, ~14 files of edits in bb.io alone. |
